try {
// This file was automatically generated from poll.soy.
// Please don't edit this file by hand.

if (typeof Confluence == 'undefined') { var Confluence = {}; }
if (typeof Confluence.Templates == 'undefined') { Confluence.Templates = {}; }
if (typeof Confluence.Templates.PollMacro == 'undefined') { Confluence.Templates.PollMacro = {}; }


Confluence.Templates.PollMacro.option = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<li data-id="', soy.$$escapeHtml(opt_data.cid), '" class="poll-option collapsed', (opt_data.noVotes) ? ' no-votes' : ' votes', (opt_data.voted) ? ' voted' : '', '"><div class="vote-header"><div class="vote-percentage" style="width: ', soy.$$escapeHtml(opt_data.percentage), '%"></div><label><input type="checkbox" ', (opt_data.voted) ? 'checked' : '', ' ', (opt_data.disabled) ? 'disabled' : '', '><span class="option-name">', soy.$$escapeHtml(opt_data.title), '</span></label><span class="vote-results"><span class="vote-participants', (opt_data.noVotes) ? ' hidden' : '', ' link"><span class="vote-count">', soy.$$escapeHtml(opt_data.votes), '</span> <span class="icon-caret-down"></span></span><span class="aui-icon aui-icon-small aui-iconfont-remove link">&times;</span></span></div><div class="vote-participants-div">');
  Confluence.Templates.PollMacro.voters(opt_data, output);
  output.append('</div></li>');
  return opt_sb ? '' : output.toString();
};


Confluence.Templates.PollMacro.voters = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  var voterList37 = opt_data.voters;
  var voterListLen37 = voterList37.length;
  for (var voterIndex37 = 0; voterIndex37 < voterListLen37; voterIndex37++) {
    var voterData37 = voterList37[voterIndex37];
    output.append('<span class="voter-avatar user-hover-trigger" data-username="', soy.$$escapeHtml(voterData37.username), '" data-name="', soy.$$escapeHtml(voterData37.fullName), '"><img src="', soy.$$escapeHtml(contextPath), soy.$$escapeHtml(voterData37.avatarUrl), '" width="24" height="24"></span>');
  }
  return opt_sb ? '' : output.toString();
};
...
try {
if(!window.jQuery){jQuery=$;jQuery.now=function(){return(new Date()).getTime()}};
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
try {
AJS.contentHover=function(i,h,c,e,l){var a=$.extend(false,AJS.contentHover.opts,l);var g,k,b=$("#content-hover-"+h);if(!b.length){$(a.container).append($('<div id="content-hover-'+h+'" class="ajs-content-hover"><div class="contents"></div></div>'));b=$("#content-hover-"+h);g=b.find(".contents");g.css("width",a.width+"px").mouseover(function(){clearTimeout(d.hideDelayTimer);b.unbind("mouseover")}).mouseout(function(){f()}).click(function(m){m.stopPropagation()});$("body").click(function(){d.beingShown=false;clearTimeout(d.hideDelayTimer);clearTimeout(d.showTimer);b.hide()})}else{g=b.find(".contents")}var d=b[0];var j=function(){if(b.is(":visible")){return}d.showTimer=setTimeout(function(){if(!d.contentLoaded||!d.shouldShow){return}d.beingShown=true;var o=$(window),m=k.x-3,q=k.y+15;if(m+a.width+30>o.width()){b.css({right:"20px",left:"auto"})}else{b.css({left:m+"px",right:"auto"})}var n=(window.pageYOffset||document.documentElement.scrollTop)+o.height();if((q+b.height())>n){q=n-b.height()-5;b.mouseover(function(){clearTimeout(d.hideDelayTimer)}).mouseout(function(){f()})}b.css({top:q+"px"});var p=$("#content-hover-shadow").appendTo(b).show();b.fadeIn(a.fadeTime,function(){d.beingShown=false});p.css({width:g.outerWidth()+32+"px",height:g.outerHeight()+25+"px"});$(".b",p).css("width",g.outerWidth()-26+"px");$(".l, .r",p).css("height",g.outerHeight()-21+"px")},a.showDelay)};var f=function(){d.beingShown=false;d.shouldShow=false;clearTimeout(d.hideDelayTimer);clearTimeout(d.showTimer);clearTimeout(d.loadTimer);d.contentLoading=false;d.shouldLoadContent=false;d.hideDelayTimer=setTimeout(function(){b.fadeOut(a.fadeTime)},a.hideDelay)};$(i).filter("[data-content-hover-bound!='true']").attr("data-content-hover-bound","true").mousemove(function(m){k={x:m.pageX,y:m.pageY};if(!d.beingShown){clearTimeout(d.showTimer)}d.shouldShow=true;if(!d.contentLoaded){if(d.contentLoading){if(d.shouldLoadContent){var n=(k.x-d.initialMousePosition.x)*(k.x-d.initialMousePosition.x)+(k.y-d.initialMousePosition.y)*(k.y-d.initialMousePosition.y);if(n>(a.mouseMoveThreshold*a.mouseMoveThreshold)){d.contentLoading=false;d.shouldLoadContent=false;clearTimeout(d.loadTimer);return}}}else{d.initialMousePosition=k;d.shouldLoadContent=true;d.contentLoading=true;d.loadTimer=setTimeout(function(){if(!d.shouldLoadContent){return}g.load(c,function(){d.contentLoaded=true;d.contentLoading=false;e.call(b,h);j()})},a.loadDelay)}}clearTimeout(d.hideDelayTimer);if(!d.beingShown){j()}}).mouseout(function(){f()});return b};AJS.contentHover.opts={fadeTime:100,hideDelay:500,showDelay:700,loadDelay:50,width:300,mouseMoveThreshold:10,container:"body"};
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
try {
(function(d,e,j,h,f,c,b){d.GoogleAnalyticsObject=f;d[f]=d[f]||function(){(d[f].q=d[f].q||[]).push(arguments)},d[f].l=1*new Date();c=e.createElement(j),b=e.getElementsByTagName(j)[0];c.async=1;c.src=h;b.parentNode.insertBefore(c,b)})(window,document,"script","//www.google-analytics.com/analytics.js","ga");ga("create","UA-41012273-3");ga("set","appVersion",AJS.Meta.get("version-number"));ga("send","pageview");
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
try {
PollMacro.OptionModel=PollMacro.Backbone.Model.extend({defaults:function(){return{voters:[]}},getPercentage:function(){if(this.collection){return ~~(this.getVotes()/this.collection.getHighestVotes()*100)}return 0},hasVoted:function(a){return _.some(this.get("voters"),function(b){return b.username===a.username})},getVotes:function(){return this.get("voters").length},toggleVote:function(a){this.collection._publish({operand:"VOTE",operator:this.hasVoted(a)?"REMOVE":"ADD",optionId:this.id});if(this.hasVoted(a)){this.unvote(a)}else{this.vote(a)}},vote:function(a){if(!this.hasVoted(a)){this.set("voters",this.get("voters").concat([a]))}},unvote:function(a){this.set("voters",_.reject(this.get("voters"),function(b){return a.username===b.username}))}});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
try {
PollMacro.OptionCollectionComparators={HighestVoted:function(a){return -1*a.getVotes()},OrderAdded:function(a){return +a.cid.substr(1)},Shuffle:function(a){return Math.random()+a.getVotes()*0}};
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
try {
PollMacro.OptionCollection=PollMacro.Backbone.Collection.extend({model:PollMacro.OptionModel,url:function(){return contextPath+"/rest/poll/1/poll/"+this.pollId},comparator:PollMacro.OptionCollectionComparators.OrderAdded,initialize:function(b,a){this.pollId=a.pollId;if(this.pollId){this._socket=$.atmosphere.subscribe({url:contextPath+"/plugins/servlet/poll/"+this.pollId,trackMessageLength:true,onMessage:_.bind(this.handleMessage,this)})}},getTotalVotes:function(){return this.reduce(function(a,b){return a+b.getVotes()},0)},getHighestVotes:function(){return this.size()?this.max(function(a){return a.getVotes()}).getVotes():0},getOptionCids:function(){return this.map(function(a){return a.cid})},parse:function(a){return a.options},handleMessage:function(d){try{d=JSON.parse(d.responseBody)}catch(b){}if(d){if(d.operand==="OPTION"){if(d.operator==="ADD"){var a=this.find(function(e){return !e.id&&e.get("title")===d.optionTitle});if(a){a.set("id",d.optionId)}else{this.add({id:d.optionId,title:d.optionTitle})}}else{if(d.operator==="REMOVE"){this.remove(d.optionId)}else{throw"IllegalArgumentException"}}}else{if(d.operand==="VOTE"){var c=this.get(d.optionId);if(d.operator==="ADD"){c.vote(d.user)}else{if(d.operator==="REMOVE"){c.unvote(d.user)}else{throw"IllegalArgumentException"}}}else{throw"IllegalArgumentException"}}}},_publish:function(a){a.pollId=this.pollId;if(this._socket){this._socket.push({data:JSON.stringify(a)})}},addOption:function(a){this.add(a);this._publish({operand:"OPTION",operator:"ADD",optionTitle:a.title})},removeOption:function(a){this.remove(a);this._publish({operand:"OPTION",operator:"REMOVE",optionId:a.id})},sortType:function(){if(this.comparator==null){return null}var b=PollMacro.OptionCollectionComparators;for(var a in b){if(b.hasOwnProperty(a)){if(b[a]===this.comparator){return a}}}return null},sortByType:function(a){if(PollMacro.OptionCollectionComparators[a]){this.comparator=PollMacro.OptionCollectionComparators[a];this.sort()}},sortByDefaultSort:function(){this.sortBy("OrderAdded")}});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
try {
PollMacro.PollView=PollMacro.Backbone.View.extend({events:{"click [type=checkbox]":"onClickOption","submit form":"onSubmit","click .vote-participants":"onClickSeeWho","click .aui-iconfont-remove":"onClickRemove","click .poll-sort":"onClickSort","mouseenter .poll-sort":"onMouseenterSort"},transEndEventNames:{WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend",OTransition:"oTransitionEnd",msTransition:"MSTransitionEnd",transition:"transitionend"},template:Confluence.Templates.PollMacro.option,initialize:function(){_.bindAll(this,"updateOrder_transitionCleanup");this.avatarUrl=this.$el.data("user-avatar-url");this.fullName=this.$el.data("user-full-name");this.collection.on("change add remove",function(){this.renderAll()},this);this.collection.on("remove",this.remove,this);this.collection.on("change add",function(){this.comparator&&this.sort()},this.collection);this.collection.on("reset",this.updateOrder,this);this.disabled=!PollMacro.username;this.transitionEndEvent=this.transEndEventNames[PollMacro.Modernizr.prefixed("transition")];this.$el.toggleClass("poll-disabled",this.disabled);this.renderAll(true)},renderAll:function(a){this.collection.each(function(b){this.render(b,a)},this)},render:function(b,a){if(this.getElementFor(b).length){this.update(b)}else{this.add(b,a)}},add:function(b,a){var c=$(this.template(this.getTemplateData(b))).appendTo(this.$(".poll-options"));if(a){c.removeClass("collapsed")}else{_.defer(function(){c.removeClass("collapsed")})}},remove:function(a){var b=this.getElementFor(a);b.on("transitionend",function(){this.remove()}).addClass("collapsed")},update:function(a){var c=this.getTemplateData(a);var b=this.getElementFor(a);b.find(".vote-percentage").css({width:c.percentage+"%"});var d=b.find("[type=checkbox]").attr("checked",c.voted);if(c.disabled){d.attr("disabled","disabled")}else{d.removeAttr("disabled")}b.find(".option-name").text(c.title);b.find(".vote-count").text(c.votes);b.find(".vote-participants").toggleClass("hidden",c.noVotes);if(c.noVotes){b.removeClass("open")}var e=Confluence.Templates.PollMacro.voters({voters:c.voters});b.find(".vote-participants-div").html(e);if(Confluence.Binder){Confluence.Binder.userHover()}b.toggleClass("no-votes",c.noVotes);b.toggleClass("votes",!c.noVotes);b.toggleClass("voted",c.voted)},updateOrder:function(){if(PollMacro.Modernizr.csstransforms&&PollMacro.Modernizr.csstransitions){this.updateOrder_transition()}else{this.updateOrder_noTransition()}},updateOrder_transition:function(){if(this.isInOrder()){return}var a=this.collection.getOptionCids();var c=this.$(".poll-option");var d=function(j,i,h){return _.reduce(j.slice(i,h),function(k,l){return k+l},0)};var g={};c.each(function(){var h=$(this).data("id");g[h]=this.offsetHeight});var f=_.map(c,function(h){var i=$(h).data("id");return g[i]});var e=_.map(a,function(h){return g[h]});var b=this.transitionEndEvent;c.each(function(k){var p=$(this).data("id");var h=_.indexOf(a,p);var j=d(e,0,h);var n=j-d(f,0,k);var m="translateY("+n+"px)";var o=this.style[PollMacro.Modernizr.prefixed("transform")];if(o!==m&&!(n===0&&o==null)){this.style[PollMacro.Modernizr.prefixed("transform")]=m;$(this).addClass("transitioning").on(b,function l(i){if(i.target===this){$(this).removeClass("transitioning").off(b,l)}})}});this.$el.off(this.transitionEndEvent,".poll-options",this.updateOrder_transitionCleanup);this.$el.on(this.transitionEndEvent,".poll-options",this.updateOrder_transitionCleanup)},updateOrder_transitionCleanup:function(){var a=this.$(".poll-option.transitioning").length>0;if(!a){this.$(".poll-option").each(function(){var b=$(this).addClass("block-transitions");this.style[PollMacro.Modernizr.prefixed("transform")]=null;b.removeClass("block-transitions")});this.updateOrder_noTransition();this.$el.off(this.transitionEndEvent,".poll-options",this.updateOrder_transitionCleanup)}},updateOrder_noTransition:function(){if(this.isInOrder()){return}var a=this.collection.getOptionCids();var b=_.map(a,function(d){return this.getElementFor(d)[0]},this);var c=$.fn.detach?"detach":"remove";$(b)[c]().appendTo(this.$(".poll-options"))},isInOrder:function(){var a=_.map(this.$(".poll-option"),function(c){return $(c).data("id")});var b=this.collection.getOptionCids();return _.isEqual(a,b)},getElementFor:function(a){var b=typeof a==="string"?a:a.cid;return this.$('[data-id="'+b+'"]')},getTemplateData:function(a){return _.extend(a.toJSON(),{cid:a.cid,voted:a.hasVoted({username:PollMacro.username}),votes:a.getVotes(),percentage:a.getPercentage(),noVotes:!a.getVotes(),disabled:this.disabled})},onClickSort:function(b){var c=$(b.currentTarget).data("sort-type");if(this.collection.sortType()===c){this.collection.sortByDefaultSort()}else{this.collection.sortByType(c)}var a=this.$('.poll-sort[data-sort-type="'+this.collection.sortType()+'"]');a.attr("aria-pressed","true");this.$(".poll-sort").not(a).removeAttr("aria-pressed")},onMouseenterSort:function(a){this.$(".poll-header-tools").attr("data-tooltip",$(a.currentTarget).attr("data-tooltip"))},onClickOption:function(b){if(this.disabled){return}var c=$(b.target).closest("li").data("id");var a=this.collection.getByCid(c);if(a){if(!a.id){console.warn("Can't vote/unvote on an option that doesn't exist!")}a.toggleVote({username:PollMacro.username,avatarUrl:this.avatarUrl,fullName:this.fullName})}else{console.warn("Can't vote/unvote on an option that doesn't exist!")}},onClickSeeWho:function(a){a.preventDefault();$(a.target).closest("li").toggleClass("open")},onSubmit:function(c){c.preventDefault();if(this.disabled){return}var a=this.$(".poll-option-add");var b=a.val();if($.trim(b)){a.val("");this.collection.addOption({title:b})}},onClickRemove:function(b){b.preventDefault();if(this.disabled){return}var c=$(b.target).closest("li").data("id");var a=this.collection.getByCid(c);if(a){if(!a.id){console.warn("Can't remove an option that doesn't exist!")}this.collection.removeOption(a)}else{console.warn("Can't remove an option that doesn't exist!")}}});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
try {
(function(){function a(){$(".poll-macro").each(function(){var d=$(this);var c=d.data("poll");c=typeof c==="string"?JSON.parse(c):c;var b=(c&&c.options)||[];var g=d.data("macro-id");if(g){g=+g}var f=new PollMacro.OptionCollection(b,{pollId:g});var e=new PollMacro.PollView({collection:f,el:this})})}if(window.ConfluenceMobile){$=window.$||Zepto;contextPath=ConfluenceMobile.AppData.get("confluence-context-path");ConfluenceMobile.contentEventAggregator.on("displayed",a)}else{$=window.$||jQuery;$(a)}})();
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
